...
export const ArticleModal: React.FC<ArticleModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs overflow-y-auto p-2 sm:p-4 md:p-6 flex justify-center items-start animate-in fade-in duration-200">
      ...
      {/* Modal Card in Geometric Balance Style */}
      <div className="bg-white dark:bg-slate-900 rounded-sm max-w-4xl w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden my-6 transition-colors">
        
        {/* Sticky Modal Header Actions */}
        <div className="sticky top-0 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-mono hidden sm:inline">• {article.date}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1.5 rounded-sm text-xs flex items-center gap-1 transition ${
                isPlayingAudio ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
              title="Simulate audio podcast readout"
            >
              {isPlayingAudio ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
              <span className="hidden md:inline font-bold uppercase text-[10px]">{isPlayingAudio ? 'Stop Audio' : 'Listen (8 min)'}</span>
            </button>
            ...
            <button
              ...
              className={`p-1.5 rounded-sm transition ${
                isBookmarked ? 'bg-amber-500 text-slate-950' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
        ...
        {/* Audio Player Bar Simulation if active */}
        {isPlayingAudio && (
        ...
        <div className="p-6 sm:p-8 md:p-10 space-y-6">
          ...
          {/* Author Byline & Stats */}
          <div className="flex flex-wrap items-center justify-between gap-4 py-3 border-y border-slate-100 dark:border-slate-800 text-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">{article.author.role} • {article.author.organization}</p>
              </div>
            </div>

            <div className="flex items-center gap-4 text-slate-500 dark:text-slate-400 font-mono">
              <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-amber-500" /> {article.readTime}</span>
              <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5 text-amber-500" /> {article.views.toLocaleString()} Reads</span>
              <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5 text-amber-500" /> {comments.length} Comments</span>
            </div>
          </div>

          {/* Hero Image */}
          <div className="rounded-sm overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-800">
            <img 
            ...
            {article.imageCaption && (
              <div className="p-2.5 bg-slate-50 dark:bg-slate-850 text-[11px] text-slate-500 dark:text-slate-400 italic text-center">
                {article.imageCaption}
              </div>
            )}
          </div>
          ...
          {article.techSpecs && (
            <div className="p-5 bg-slate-50 dark:bg-slate-850 rounded-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {article.techSpecs.map((spec, i) => (
                  <div key={i} className="p-2.5 bg-white dark:bg-slate-900 rounded-sm border border-slate-200 dark:border-slate-800">
                    <span className="text-[10px] text-slate-500 uppercase font-black block">{spec.label}</span>
                    <span className="font-mono font-black text-slate-900 dark:text-amber-400 text-xs sm:text-sm">{spec.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={`prose dark:prose-invert max-w-none text-slate-800 dark:text-slate-200 leading-relaxed space-y-4 ${
            ...
            {article.content.split('\n\n').map((paragraph, index) => {
              if (paragraph.trim().startsWith('###')) {
                return (
                  <h3 key={index} className="text-xl font-black text-slate-900 dark:text-white pt-4">
              ...
              if (paragraph.trim().startsWith('####')) {
                return (
                  <h4 key={index} className="text-lg font-bold text-slate-800 dark:text-slate-200 pt-2">
                    {paragraph.replace('####', '').trim()}
                  </h4>
                );
              }
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
            {article.tags.map((tag) => (
              <span key={tag} className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-sm">
                #{tag}
              </span>
            ))}
          </div>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
            ...
            <form onSubmit={handleAddComment} className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm text-xs sm:text-sm focus:outline-none focus:border-amber-500 text-slate-900 dark:text-white"
              />
              <button
                type="submit"
            ...
            <div className="space-y-2.5">
              {comments.map((comment, i) => (
                <div key={i} className="p-3 bg-slate-50 dark:bg-slate-850 rounded-sm border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300">
                  <div className="flex items-center justify-between text-[10px] text-slate-400 mb-1">
                    <span className="font-bold text-slate-800 dark:text-slate-200">Façade Engineering Peer #{i + 1}</span>
                    <span className="font-mono">Verified Consultant</span>
                  </div>
                  <p>{comment}</p>
                </div>
              ))}
